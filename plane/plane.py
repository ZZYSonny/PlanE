...
class TriEncoder(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.pe = tgnn.PositionalEncoding(
            config.dim_plane_pe, base_freq=1 / 64
        )
        self.lin_node = MLP(
            2 * config.dim + 2 * config.dim_plane_pe,
            config.dim,
            drop=config.drop_enc,
            factor=config.flags_mlp_factor,
        )
    ...
    def forward(self, data, h_g, h_e):
        ...
        h_cycle_edge = torch.zeros(
            (id_e.size(0), self.config.dim), device=h_g.device
        )
        ...
        pre_agg = self.lin_node(
            torch.cat(
                [h_g[id_u], h_cycle_edge, self.pe(code1), self.pe(code2)],
                dim=1,
            )
        )
    ...
    def __init__(self, config: ModelConfig) -> None:
        ...
        if config.flags_plane_gine_type == "complete":
            ...
            self.update = GINEMLPConv(
                MLP(
                    config.dim + config.dim_plane_pe,
                    config.dim,
                    drop=config.drop_edg,
                    factor=config.flags_mlp_factor,
                ),
                nn.Identity(),
                train_eps=True,
        ...
        self.mlp_after_update = MLP(
            config.dim,
            config.dim,
            drop=config.drop_rec,
            factor=config.flags_mlp_factor,
            norm="None",
        )
    ...
    def forward(self, data, h_spqr):
        ...
        h_b = self.read(
            (
                h_spqr,
                torch.zeros(
                    (data.b_batch.size(0), self.config.dim),
...
class CnearBEncoder(nn.Module):
    ...
    def forward(self, data, h_g, h_b):
        ...
        return self.mp((h_b, h_g), data.bc_edge_index)
...
class CRecSubEncoder(nn.Module):
    ...
    def forward(self, data, h_g, h_b):
        max_order = max(data.b_order.max().item(), data.c_order.max().item())
        h_g = h_g.clone()
        h_b = h_b.clone()
...
class PlaneLayer(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.flag_aggr_planar_readout = (
            "cr" in config.plane_terms or "cnb" in config.plane_terms
        )
        self.flag_aggr_global_readout = "gr" in config.plane_terms
        self.flag_compute_planar_readout = self.flag_aggr_planar_readout
        self.flag_compute_b = (
            self.flag_aggr_b or self.flag_compute_planar_readout
        )
        ...
        if self.flag_aggr_global_readout:
            self.encoder_gr = tgnn.DeepSetsAggregation(
                nn.Identity(),
                MLP(
                    config.dim,
                    config.dim,
                    drop=config.drop_agg,
                    factor=config.flags_mlp_factor,
                    norm=config.flags_norm_before_com,
                ),
            )
                ...
                if config.flags_plane_gine_type == "complete":
                    self.aggr_neigh = GINEMLPConv(
                        MLP(
                            ...
                            factor=(
                                -1
                                if config.flags_mlp_factor == -1
                                else config.flags_mlp_factor // 2
                            ),
                            drop=config.drop_edg,
                        ),
                        MLP(
    ...
    def forward(self, data, h_g):
        ...
        if self.flag_aggr_neigh:
            ...
            if h_e is not None:
                ...
                aggs.append(
                    self.aggr_neigh(
                        (h_g, h_g), edge_index=data.edge_index, edge_attr=h_e
                    )
                )
            else:
                # No edge features
        ...
        if self.flag_aggr_global_readout:
            ...
            h_gr = self.encoder_gr(h_g, data.batch, dim_size=data.num_graphs)
