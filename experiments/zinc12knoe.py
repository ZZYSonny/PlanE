...
if arg is None:
    ...
    parser.add_argument('--dim_hidden', type=int, default=72)
...
def get_dataset():
    ...
    return (
        tgsets.ZINC(root=root, subset=True, split="train", pre_transform=transorm),
        tgsets.ZINC(root=root, subset=True, split="val", pre_transform=transorm),
        tgsets.ZINC(root=root, subset=True, split="test", pre_transform=transorm),
    )
